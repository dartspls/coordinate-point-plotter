...
def setup():
    size(800, 650) # adding a 50 point margin to the bottom of the screen to have the mode toggle "button"
    background(255)
    frameRate(30)
...
def drawButtons():
    ...
    if mode:
        fill(0, 255, 0)
        text("Mode: Add", 55, 625)
    else:
        fill(255, 0, 0)
        text("Mode: Remove", 55, 625)
    rect(0, 600, 50, 50)
    
    # draw "remove all" button
    fill(255, 0, 0)
    rect(150, 600, 50, 50)
    text("Remove all", 205, 625)
    
    # draw "toggle lines" button
    if drawLine:
        fill(0, 255, 0)
    text("Draw lines", 355, 625)
    rect(300, 600, 50, 50)
...
def checkButtons(x, y):
    if x < 50 and y > 600:
        ...
        mode = not mode
        return
    elif x > 150 and x < 200 and y > 600:
        del points[:] # delete all objects in array (list.clear() isn't in this version of python)
        return
    elif x > 300 and x < 350 and y > 600:
        global drawLine
        drawLine = not drawLine
    elif x > 450 and x < 500 and y > 600:
        global snap
        snap = not snap
    elif x > 600 and x < 650 and y > 600:
        global drawGraph
